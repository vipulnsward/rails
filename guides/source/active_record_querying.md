...
```ruby
# Give export an array of 1000 customers at a time.
...
```ruby
# Give export an array of 1000 recently active customers at a time.
...
```ruby
Book.where(created_at: Date.yesterday.all_day)
```
...
This allows you to return an empty relation so that calling methods don't have to do a check for nil.
